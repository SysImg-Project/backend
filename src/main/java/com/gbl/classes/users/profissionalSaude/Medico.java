...
// Porquê a herança aqui não é: Medico->ProfissionalSaude->User->Pessoa->Crud ?
// E sim é: Medico->ProfissionalSaude->Crud sendo que User é uma 'agregada' (diamante branco) em Medico ?
// - Para poder reaproveitar os métodos de acesso à DB já existente em User (sem ter que fazer o override em todos os métodos)
    ...
    //private int numeroRegistro; // ---> Herdados de ProfissionalSaude.java
    //private EstadoBrasil estadoConselho; // ---> Herdados de ProfissionalSaude.java
    private User user;
    ...
    public Medico(int medicoId, int numeroRegistro, String estadoConselho, int userId, String nome, String sobreNome, String datanasIso, Sexo sexo, Tipo tipo, String login, String senha) {
        //public ProfissionalSaude(int id, int numeroRegistro, String estadoConselho)
        super(medicoId, numeroRegistro, estadoConselho);
        user = new User(userId, nome, sobreNome, datanasIso, sexo, tipo, login, senha);
        inicializacaoMedico();
    }

    private void inicializacaoMedico() { // Usado pelos construtores
        log.logN("Entrou em: private void inicializacaoMedico()");
        // Faz as verificações para uso da DB:
        this.setTabelaNome("medicos");
    ...
    public RetornoCrud createTable() {
        ...
        try(Connection conn = DriverManager.getConnection(this.db.conexaoStr, this.db.conexaoLogin, this.db.conexaoSenha)) {
            ...
            comando += "numeroRegistro INT(6), ";
            comando += "estadoConselho TINYINT, ";
            ...
            log.erroN("LOCAL DO ERRO: public RetornoCrud Medico.createTable()");
    ...
    public RetornoCrud create() { // 'int' Pq retorna o número do índice inserida na tabela
        ...
        //comando = "INSERT INTO usuarios (nome, sobrenome) VALUES ('Paulo', 'Griebler');"; // Não insere no campo 'id' pois ele é auto incrementeado ! // comando INSERT -- retorna um int
            ...
            comando += "(numeroRegistro, estadoConselho, userId)";
            //comando += " VALUES (\"\", \"\", \"0000-01-01\", 0, 0, \"\", \"\");";
            comando += " VALUES ("+getNumeroRegistro()+", "+getEstadoConselhoInt()+", "+user.getId()+");";
            log.dbN(comando);
            ...
            log.erroN("LOCAL DO ERRO: public RetornoCrud Medico.create()");
            log.erroN(e.getSQLState()); // Código do Erro
            log.erroN(e.toString()); // Info do Erro
        ...
        try(Connection conn = DriverManager.getConnection(this.db.conexaoStr, this.db.conexaoLogin, this.db.conexaoSenha)) {
            ...
            if (result.next()) { // Significa que tem resultados
                do { // Recebe as colunas que estão dentro desta linha
                    ...
                    this.setNumeroRegistro(result.getInt("numeroRegistro"));
                    this.setEstadoConselhoInt(result.getInt("estadoConselho"));
            ...
            log.erroN("LOCAL DO ERRO: public RetornoCrud Medico.read()");
            log.erroN(e.getSQLState()); // Código do Erro
            log.erroN(e.toString()); // Info do Erro
    ...
    public RetornoCrud update(int id) {
        ...
        try(Connection conn = DriverManager.getConnection(this.db.conexaoStr, this.db.conexaoLogin, this.db.conexaoSenha)) {
            ...
            comando += "numeroRegistro="+getNumeroRegistro()+", ";
            comando += "estadoConselho="+getEstadoConselhoInt()+", ";
    ...
    public RetornoCrud delete(int id) {
        ...
        try(Connection conn = DriverManager.getConnection(this.db.conexaoStr, this.db.conexaoLogin, this.db.conexaoSenha)) {
            ...
            log.erroN("LOCAL DO ERRO: public RetornoCrud Medico.delete()");
            log.erroN(e.getSQLState()); // Código do Erro
            log.erroN(e.toString()); // Info do Erro
            RetornoCrud ret = new RetornoCrud(false, e.getSQLState()+" - "+e.toString(), null);
            return ret;
    ...
    public JSONObject toJSON() {
        ...
        obj += "\"id\":"+this.getId()+",";
        obj += "\"numeroRegistro\":"+this.getNumeroRegistro()+",";
        obj += "\"estadoConselho\":\""+this.getEstadoConselhoSigla()+"\" ,";
    ...
    public String toString() {
        return "Medico: ["+super.toString()+", "+this.user.toString()+"]";
    }
