...
public class Paciente extends Crud {
// Porquê a herança aqui não é: Paciente->User->Pessoa->Crud ?
// E sim é: Paciente->Crud sendo que User é uma 'agregada' (diamante branco) em Paciente ?
// - Para poder reaproveitar os métodos de acesso à DB já existente em User (sem ter que fazer o override em todos os métodos)
// - Observe que como Paciente vai ter uma tabela própria na DB ela precisa herdar os métodos de Crud para tal
    ...
    public Paciente() {
        super();
        user = new User();
        inicializacaoPaciente();
    }

    public Paciente(int userId, String nome, String sobreNome, String datanasIso, Sexo sexo, Tipo tipo, String login, String senha) {
        user = new User(userId, nome, sobreNome, datanasIso, sexo, tipo, login, senha);
        inicializacaoPaciente();
    }

    private void inicializacaoPaciente() { // Usado pelos construtores
        log.logN("Entrou em: private void inicializacaoPaciente()");
        // Faz as verificações para uso da DB:
        this.setTabelaNome("pacientes");
    ...
    public RetornoCrud createTable() {
        ...
        try(Connection conn = DriverManager.getConnection(this.db.conexaoStr, this.db.conexaoLogin, this.db.conexaoSenha)) {
            ...
            log.erroN("LOCAL DO ERRO: public RetornoCrud Paciente.createTable()");
    ...
    public RetornoCrud create() { // 'int' Pq retorna o número do índice inserida na tabela
        ...
        //comando = "INSERT INTO usuarios (nome, sobrenome) VALUES ('Paulo', 'Griebler');"; // Não insere no campo 'id' pois ele é auto incrementeado ! // comando INSERT -- retorna um int
            ...
            log.erroN("LOCAL DO ERRO: public RetornoCrud Paciente.create()");
            log.erroN(e.getSQLState()); // Código do Erro
            log.erroN(e.toString()); // Info do Erro
    ...
    public RetornoCrud read(int id) {
        ...
        try(Connection conn = DriverManager.getConnection(this.db.conexaoStr, this.db.conexaoLogin, this.db.conexaoSenha)) {
            ...
            log.erroN("LOCAL DO ERRO: public RetornoCrud Paciente.read()");
            log.erroN(e.getSQLState()); // Código do Erro
            log.erroN(e.toString()); // Info do Erro
    ...
    public RetornoCrud delete(int id) {
        ...
        try(Connection conn = DriverManager.getConnection(this.db.conexaoStr, this.db.conexaoLogin, this.db.conexaoSenha)) {
            ...
            log.erroN("LOCAL DO ERRO: public RetornoCrud Paciente.delete()");
            log.erroN(e.getSQLState()); // Código do Erro
            log.erroN(e.toString()); // Info do Erro
            RetornoCrud ret = new RetornoCrud(false, e.getSQLState()+" - "+e.toString(), null);
            return ret;
    ...
    public String toString() {
        return "Paciente: [id: "+this.getId()+", "+this.user.toString()+"]";
    }
